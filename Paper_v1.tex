...
Thus, reasonable predictions from sloppy models may be possible, even if the parameters or structure is uncertain, 
if a few critical directions in parameter space are properly constrained. 
Typical ODE models can also require significant mechanistic information.
Alternatively, data-driven approaches \citep{Cirit:2012kx} or logical model formulations \citep{Morris:2011ys} 
are emerging paradigms that constrain model structure by the availability of training and validation data. 
These are interesting techniques because the data naturally informs model structure, which can be expanded as more data becomes available, and limits the 
identification challenge. Although not mechanistic, these models can simulate surprisingly rich system responses in the absence of mechanism [REFHERE]. 
...
and later Hockin \emph{et al.} \citep{Hockin:2002aa}, who developed and then refined a highly mechanistic model of extrinsic coagulation. 
