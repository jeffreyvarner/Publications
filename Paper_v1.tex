...
Modeling approaches differ in their degree of detail, and the choice of approach is often determined by prior knowledge, or model requirements \citep{2012_kholodenko_kolch_SciSig}. 
...
Since Bailey's complex biology with no parameters hypothesis, Sethna and coworkers have shown that model behavior and predictive ability is typically 
...
Thus, accurate model predictions from a sloppy model may be possible, even if the parameters or mechanistic structure is not well understood, 
if the critical directions in parameter space are properly constrained. 
...
While not mechanistic, these models can simulate surprisingly rich system responses in the absence of mechanism [REFHERE]. 
However, modeling dynamics using logical models is not straightforward. 
Thus, a potential third approach could be a combination of ODEs and logical models, where mechanistic information was used when available, 
and missing or incomplete knowledge encoded using causal local rules.  

In this study, we developed a hybrid approach which combined ODEs and logical rules to model a well studied biochemical network, 
...
Thrombin generation involves three phases, initiation, amplification and termination \cite{GOLDHABER2006, BRUMMEL2002}.
...
Initially, thrombin is produced upon cleavage of prothrombin by fluid phase activated factor X (FXa), which itself has been activated by FVIIa \cite{BUTENAS2002a}. 
...
There are many mathematical models of the human coagulation cascade in the literature.
Previous coagulation models have typically been formulated as systems of nonlinear ordinary differential equations, 
often using mass-action kinetics to describe the rates of biochemical conversions [REFHERE].
Previous ODE coagulation models from our laboratory \citep{Luan:2007aa,2010_luan_varner_MolBioSys}, 
were built upon the pioneering studies of Jones and Mann \citep{Jones:1994aa}, 
and later Hockin \emph{et al.} \citep{Hockin:2002aa}, who developed and subsequently refined a highly mechanistic model of the extrinsic coagulation cascade. 
Other laboratories have also expanded upon Hockin \emph{et al.}, for example Chatterjee \emph{et al.,} augmented the Hockin model to include 
the intrinsic pathway, and the role of thrombin in clot formation \citep{Chatterjee:2010aa}. 
More recently, modeling studies have explored other biochemical or biophysical aspects of coagulation, such as the biochemical networks inside platelets \citep{Stalker:2013aa}, 
multi-scale models of clot formation \citep{Leiderman:2014aa, Bannish:2014ab}, and the transport of material inside growing clots \citep{Voronov:2013aa}.
However, most of these previous studies share the common feature of a mechanism based description of coagulation. 
Such a description is possible because blood, while enormously complex, can be systematically interrogated. 
However, other biochemical networks, such as the intracellular networks which mediate cancer, are more difficult to probe and model.  
Towards this need, we have proposed a hybrid approach which combined ODEs and logical rules that can be used to model 
biochemical processes where mechanistic information is difficult or impossible to obtain. 
...
Our hypothesis is that coagulation represents a best case, if our approach fails in this proof-of-concept study, it will never be acceptable in model difficult cases.
...
We identified the reduced order coagulation model parameters from in vitro extrinsic data sets of thrombin generation initiated by TF/FVIIa in the presence of ATIII, with and without the protein C pathway. We then validated the model using thrombin data sets from multiple laboratories, for both normal and hemophilia coagulation.
The reduced order hybrid approach produced a surprisingly predictive coagulation model, suggesting this framework could potentially be used to model other biochemical networks
