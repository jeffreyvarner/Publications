...
Theoretically, an integration rule is simply a function whose domain is a set of transfer function inputs, and whose range is $[0,1]$.
Thus, integration rules other than max/min could be used, such as the mean or the product, assuming the range of the transfer functions is always $[0,1]$.
...
However, the only mathematical requirement for a transfer function is to map a non-negative continuous or categorical variable into the 
range $[0,1]$. Thus, many other function types e.g., tanh are also possible.
...
Thus, platelet and fXI activation may be integrated.
Lastly, to make the model more clinically relevant, we should include the biochemical processes driving clot formation, and clot dissolution (fibrinolysis).
